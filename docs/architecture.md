...
- `interview_bank_ops_actions`
  保存管理员需要跟进的题库运营动作，记录动作类型、状态、优先级、来源、去重键、标题、原因、关联组合、关联原子、轻量证据和创建人；已支持管理员手工创建、open 队列读取、健康诊断/索引状态/真实检索运营候选预览，以及管理员显式保存选中候选。候选生成不落库，候选保存只写治理动作，不自动修题、归档或重建索引。
