...
- 按当前验收约束，本阶段未运行测试套件，也未进行额外编译校验；最终以 Docker 重建后的真实浏览器交互为准。
- 仍待完成：在真实浏览器中制造一次 `needs_refocus` 或 `off_topic` 投影，确认下一轮 Agent 的承接方式改变，且页面只显示粗粒度方向提示，不泄露内部 ID。

## 后续工作

1. Docker 重建 `api` 与 `agent`，让本地页面加载本阶段代码。
2. 用真实浏览器完成一轮“偏离/停滞 → 下一轮回到故障”的交互证据。
3. 在同一浏览器会话中观察 `pending → running → completed` 的图标与行高亮变化。
4. 若验证通过，提交本阶段 commit：`feat(scenario): 回注跨轮方向教学信号`。
