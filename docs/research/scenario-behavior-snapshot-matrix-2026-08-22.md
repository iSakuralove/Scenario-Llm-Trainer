...
| BS-06 | 连续卡住：在同一会话连续三轮只发送“我不知道”“还是不懂”“继续告诉我”。 | Hint 按 `0→1→2→3→4` 逐级提升；学生提交新证据或正确因果后允许回落；Hint 作为系统帮助，不计入学生独立证据；线索常驻但不直接等同答案。 | 跳级释放完整答案；把 Hint 当成学生已发现的证据；Hint 无限增加或在有进展后不回落；显示内部 `hint_state`/评分字段。 | **已捕获（部分递进，静态链路已复核）**。真实会话观察到 Hint 1→3；Sol 只读审查确认固定题库、Python reducer、Go 审批、SSE 和前端均可达 Hint 4 及单步回落；独立 Hint 4/回落浏览器 artifact 仍待验收。artifact：`.omo/evidence/phase6-hint-escalation-2026-08-22.md`。 |
...
| BS-09 | 修复验证：先完成 timeout 止血，再发送“改完以后看看怎么样”，随后补充“504 降了但 lock_wait 还是高”。 | 分层验证表面结果、P99/lock wait、重试/幂等和最终业务结果；`repair_status` 从 `partial` 到 `sufficient` 只能在修复动作和声明的验证步骤覆盖后发生；能识别“504 降低但 lock_wait 仍高”的未闭环分叉。 | 只回复“改回 10 秒就好了”；把单一指标改善当作完整闭环；把验证前的结论写成完成。 | **已捕获部分证据，在线 repair_status 提交待验收**。活动会话历史明确区分 timeout 止血、锁等待源头和分层验证，并识别“504 降低但 lock_wait 仍高”的未闭环分叉；新提交仍受旧 API 镜像契约问题影响。artifact：`.omo/evidence/phase6-bs09-repair-validation-2026-08-22.md`。 |
