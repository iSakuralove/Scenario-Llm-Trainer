...
- `hydrate` 对历史消息的 `response_meta` 改为可选读取，并只把数组形态的 `run_events` 送入归一化；旧版或损坏消息不会让整页刷新恢复失败。
- V2 `turn_*` 事件进一步做判别校验：拒绝数组 payload、错误类型的 retryable/error_code、非法 repair_status 和不完整 QuickAction，避免坏的完成/失败事件进入学生视图或刷新游标。
...
- 已收紧前端续接边界：客户端超时、响应建立前的网络失败和 HTTP/SSE/协议错误不再复用同一 `request_id` 自动重放，只有进入响应体 reader 后的底层读取中断保留一次续接路径；当前页面刷新复查仍停在第 24 轮且没有自动重放。
...
- `AgentRun` 失败态现在显式清空 raw reasoning chunks、active 标记和计时信息；即使 `turn_failed` 与调试旁路同一帧到达，也不会在清理窗口残留内部思维内容。
...
- 真实活动会话补了导师回显防线的部分 artifact：总结请求得到独立的因果/修复/验证摘要，不等于用户原文；上游失败 fallback 和 Go 最终拒绝的独立注入仍待验收，证据记录在 `phase6-bs17-reply-echo-2026-08-22.md`。
- 真实活动会话补了修复验证部分 artifact：导师把恢复 timeout 定义为止血，要求继续观察 504/P99、lock_wait/db_time、重试量和最终写库结果，并明确“504 降低但 lock_wait 仍高”不是完整闭环；在线 `repair_status` 提交仍待旧 API 镜像更新，证据记录在 `phase6-bs09-repair-validation-2026-08-22.md`。
